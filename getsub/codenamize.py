...
def int_to_base62(num):
    """
    Convert integer to base62 string (digits 0-9, letters A-Z, letters a-z).
    """
    if num < 0:
        num = -num
    if num == 0:
...
def codenamize_particles(obj = None, adjectives = 1, max_item_chars = 0, hash_algo = 'md5', suffix = 0):
    """
    ...
    Args:
        ...
        suffix (int): number of base36 characters to append as a suffix (default 0)
    ...
    if obj is None:
        if suffix and suffix > 0:
            modulus = 62 ** suffix
            g = math.gcd(total_words, modulus)
            lcm = (total_words * modulus) // g
            return lcm
        return total_words
    ...
    # Append base36 suffix if requested
    if suffix and suffix > 0:
        # take a value in range [0, 62**suffix - 1] from obj_hash
        modulus = 62 ** suffix
        val = obj_hash % modulus
        s = int_to_base62(val)
        # pad with leading '0' to ensure exact length
        if len(s) < suffix:
            s = s.rjust(suffix, '0')
        codename_particles.append(s)

    return codename_particles


def codenamize_space(adjectives, max_item_chars, hash_algo = 'md5', suffix = 0):
    """
    Returns the size of the codename space for the given parameters.
    """

    return codenamize_particles(None, adjectives, max_item_chars, hash_algo, suffix)


def codenamize(obj, adjectives = 1, max_item_chars = 0, join = "-", capitalize = False, hash_algo = 'md5', suffix = 0):
    """
    ...
    Args:
        ...
        adjectives (int): Number of adjectives to use (default 1).
        max_item_chars (int): Max characters of each part of the codename (0 for no limit).
        join: (string) String used to join codename parts (default "-").
    ...
    codename_particles = codenamize_particles(obj, adjectives, max_item_chars, hash_algo, suffix)
...
def print_test():
    ...
    Shows codename samples and prints codename-space sizes.
    Also performs small distinct-output tests.
    """
    print("OBJ       ADJ0-MAX5    ADJ1-MAX5         ADJ2-MAX5  ADJ-0, ADJ-1, ADJ-2 (capitalized, empty join character)")
    for v in range(100001, 100010):
        print("%6s  %11s  %11s %17s  %s, %s, %s" % (
            v,
            codenamize(v, 0, 5),
            codenamize(v, 1, 5),
            codenamize(v, 2, 5),
            codenamize(v, 0, 0, "", True),
            codenamize(v, 1, 0, "", True),
            codenamize(v, 2, 0, "", True),
        ))

    print("\codenamize SPACE SIZES (theoretical achievable distinct outputs)")
    for a in range(0, 3):
        for m in (3, 4, 5, 6, 7, 0):
            no_suffix = codenamize_space(a, m, hash_algo='md5', suffix=0)
            suffix4   = codenamize_space(a, m, hash_algo='md5', suffix=4)
            print("%d adj (max %d chars) = %d combinations (no suffix), %d combinations (suffix=4)" %
                  (a, m, no_suffix, suffix4))

    print("\nEXPERIMENTAL DISTINCT-OUTPUT CHECKS")
    # small brute-force tests: see how many distinct results we get for ranges used earlier
    l1 = set([ codenamize(a, 1, 3) for a in range(0, 2760 + 17) ])
    l2 = set([ codenamize(a, 2, 3) for a in range(0, 66240 + 17) ])
    ...
    s = 4
    l1s = set([ codenamize(a, 1, 3, suffix=s) for a in range(0, 2760 + 17) ])
    print("  (*, 1 adj, max 3, suffix=%d) => %d distinct results (space size is %d)" %
          (s, len(l1s), codenamize_space(1, 3, suffix=s)))

    print("\nSAMPLE VERIFICATION (must match previous expectations):")
...
def main():
    ...
    parser.add_argument('--suffix', dest='suffix', action='store', type=int, default=0,
                        help='append base36 suffix of this length (0 for none)')
    ...
    if args.space:
        print(codenamize_space(args.prefix, args.maxchars, args.hash_algo, args.suffix))
        return
    ...
    for o in args.strings:
        print(codenamize(o, args.prefix, args.maxchars, args.join, args.capitalize, args.hash_algo, args.suffix))
